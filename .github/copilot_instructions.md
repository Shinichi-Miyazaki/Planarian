...
## 20260116 追加の指示
現状の手法ではやはり検出率が低い場合がある。特に夜間画像で顕著。
プラナリアは背景に対して暗い物体である。
辺縁は整であり、凸凹はしていない。
バックグラウンドはかなり不均一なので、rolling ball 等の背景補正は有効である可能性が高い。
パラメータを手動で調整するのは限界があるため、背景補正を自動化してから二値化を行う方法を検討してください。
人の目から見てプラナリアはわかりやすいので、インタラクティブにプラナリアの部分をクリックして、それをdetectできるように
という教師あり学習の要素も入れてもいいかもしれません。

## 20260205 
どうもうまくいかないので、現存のコードは残したままで、セマンティックセグメンテーションをdeep learningで行う方法を検討してください。
動画データはたくさんあるので、学習データセットの作成は比較的容易です。
ラベリングから行えるGUIの作成、学習、推論までの一連の流れをスクリプト化してください。
のちに新しいデータも同じモデルで推論出きるように、モデルの保存と読み込みもお願い致します。
