...
3. **解析済みフォルダの自動スキップ**
   - 既に`segmentation_analysis/`フォルダが存在する場合は自動的にスキップ
   - 既存の解析結果を統合解析に利用
   - 部分的な解析結果（必須ファイルが欠けている）は警告を出して再解析

4. **強制再解析オプション**
   - 解析済みフォルダも再度解析する場合にチェック
   - 解析パラメータを変更した際に便利
...
6. **統合解析結果の出力先指定**
   - 統合解析結果を任意のフォルダに保存可能
   - 未指定時は親フォルダ直下の`batch_summary/`に保存
...
#### 統合結果出力先を選択（オプション）
- 「統合結果出力先」の「参照」ボタンをクリック
- 統合解析結果（`batch_summary`）を保存するフォルダを選択
- 未指定時は親フォルダ直下の`batch_summary/`に自動保存
...
各フォルダの解析状態が色分けで表示されます：

- **✓ 解析済み**（緑色）: `segmentation_analysis/`フォルダと全ての必須ファイルが存在
- **未解析**（赤色）: まだ解析されていないフォルダ

#### 強制再解析オプション

「解析済みフォルダも強制再解析」チェックボックス：
- チェックなし（デフォルト）: 解析済みフォルダをスキップして既存データを使用
- チェックあり: 解析済みフォルダも含めて全て再解析

**使用シーン**:
- 解析パラメータ（時間間隔、昼夜の開始時刻など）を変更した場合
- 解析アルゴリズムを改善した後に再計算したい場合
...
1. **個別フォルダ解析**（順次実行）
   - 未解析フォルダ: セグメンテーション推論と行動解析を実行
   - 解析済みフォルダ: スキップして既存データを使用（強制再解析がOFFの場合）
   - 各フォルダ内に`segmentation_analysis/`を作成
   - 部分的な解析結果（必須ファイルが欠けている）: 警告を出して再解析
...
2. **統合解析**（2つ以上の実験がある場合のみ）
   ...
   - 統合プロット作成
   - R解析用CSV出力
   - 指定フォルダまたはデフォルトの`batch_summary/`に保存
...
**結果の内訳:**
- **成功**: 新規に解析を実行して完了したフォルダ
- **スキップ**: 既に解析済みで既存データを使用したフォルダ
- **失敗**: エラーが発生したフォルダ
...
統合解析結果の出力先（デフォルト: 親フォルダ直下の`batch_summary/`）：
