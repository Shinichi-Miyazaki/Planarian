...
3. Excelファイルを指定（オプション）：フォルダ名と測定日時のリスト
4. 統合結果出力先を選択（オプション）：未指定時は親フォルダ/batch_summary
5. 画像数閾値を設定（デフォルト: 1000枚以上）
6. 「フォルダを検索」をクリック
7. 検出されたフォルダが解析状態付きで表示（✓ 解析済み / 未解析）
8. 必要に応じて「解析済みフォルダも強制再解析」をチェック
9. 検出されたフォルダごとに測定日時を編集（ダブルクリック）
10. 「バッチ実行」で一括解析開始
11. 各フォルダ内に`segmentation_analysis`フォルダが作成され、結果が保存される
12. 2つ以上のフォルダがある場合、統合解析結果が出力される

**新機能:**
- **解析済みフォルダのスキップ**: 既に解析済みのフォルダは自動的にスキップし、既存データを統合解析に利用
- **強制再解析**: パラメータ変更時などに解析済みフォルダも再度解析
- **統合結果の出力先指定**: 統合解析結果を任意のフォルダに保存可能
- **スムージングパラメータ調整**: 時系列プロットの平滑化をGUIから調整可能（個別/時間ビン/Mean）
- **統計検定**: 明期vs暗期のボックスプロットに対応のあるt検定（paired t-test）とp値を表示
...
### 高度な機能
- **[Excelバッチ解析ガイド](docs/EXCEL_BATCH_ANALYSIS_GUIDE.md)**: Excelファイルからの一括解析
- **[解析済みフォルダスキップ機能](docs/SKIP_ANALYZED_FOLDERS_FEATURE.md)**: 効率的なバッチ解析（NEW）
...
│   └── labels/
├── docs/
│   ├── EXCEL_BATCH_ANALYSIS_GUIDE.md    # Excelバッチ解析ガイド
│   ├── SKIP_ANALYZED_FOLDERS_FEATURE.md # スキップ機能の実装詳細（NEW）
│   ├── BATCH_SUMMARY_ANALYSIS.md        # 統合解析の詳細
